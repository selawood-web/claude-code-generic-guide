...
| `/ccgg-audit` | Know what the gate cannot see | Deterministic checks and mutation probes first, then read-only specialist agents, then a verifier that reproduces every finding inside a worktree — never edits a tracked file |
...
are executable and valid · every feature definition meets its schema · every audit
subagent keeps its read-only boundary.
