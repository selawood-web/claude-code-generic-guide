...
echo "     .claude/ccgg-origins (the validator's record) and ~/.claude/ccgg-origins"
echo "     (the hook's, once per machine). Every session start then syncs the"
echo "     guide's skills/hooks/validator via update.sh — which runs with your"
echo "     permissions, so the hook runs nothing unless all three are set and"
echo "     the URL is in your user-level record."
