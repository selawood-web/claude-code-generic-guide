...
| 2 | 🟡 Important | dead-mechanism | The pre-compact hook prints "run /flush" **to a channel the agent never sees.** Only `SessionStart`, `UserPromptSubmit`, `UserPromptExpansion`, and `PostModelSwitch` add stdout to the agent's context. *Correction after the first live audit run:* `PreCompact` stdout is not discarded — the product hands it to the compaction model as additional summary instructions, and no `additionalContext` output path exists for that event. The effect stands: the agent cannot act on it. | `.claude/hooks/pre-compact.sh`; hooks reference, *Exit code 0*; the installed product's PreCompact executor, read by the audit verifier (report H-001). | Make the echo a real summary instruction — what to preserve — and reword the overview's "reminds it to run /flush" claim. A pre-compaction flush is the user's or the agent's own call, earlier. Verified, and fixed in the follow-up to slice 1. |
